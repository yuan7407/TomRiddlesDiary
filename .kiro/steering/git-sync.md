...
3. 收口前：运行 `scripts/ip_firewall_check.sh` + 受影响范围的测试/构建（Xcode 用 `xcodebuild`；Swift Package 才用 `swift test`）。
4. 只 stage 本任务相关文件，先检查 staged diff；密钥、本地 persona、xcuserdata、`.kiro/pet/` 永不入库。
