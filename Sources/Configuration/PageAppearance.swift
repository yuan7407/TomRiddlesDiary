...
nonisolated enum PageAppearance {
    ...
    /// 成页预告那团渗开的墨，最大半径占字高的比例（计划 E3c）。
    ///
    /// 为什么用「渗墨」表现预告：这张纸上不许有控件（决策 20），进度条或提示按钮
    /// 都不行。墨在纸上洇开是世界观内的动作——像笔尖还没落下，墨已经先到了。
    /// 它同时就是「此刻还能取消」的凭证：看到它就知道再写一笔就没事了。
    ///
    /// **位置和造型都是临时的**：现在画在页边距的左上角，因为「用户在哪写、
    /// 魂在哪回应」这个版式决定还没做（见 `MEMORY.md` 待决项）。定了之后它应该
    /// 出现在魂真正要落笔的地方。不要把当前的样子当成设计结论。
    static let commitHintRadiusRatio: Double = 0.55

    /// 成页预告那团墨的浓度。比正文淡：它是预告，不是内容。
    static let commitHintOpacity: Double = 0.3
