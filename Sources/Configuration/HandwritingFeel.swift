...
enum HandwritingFeel {
    ...
    /// 压感为 0 时墨线宽度占字高的比例。0.025 → 9 mm 的字约 0.22 mm。
    /// 不取 0，因为真实笔尖落纸就有宽度。
    static let inkMinimumWidthRatio: Double = 0.025

    /// 压感由 0 升到 1 时墨线额外增加的宽度占字高的比例。
    /// 0.098 → 满压约 0.88 mm。这个值明显比真实钢笔（约 0.3–0.6 mm）粗，
    /// 是从早期离线验证的数值反推来的，留待 A10 对着真人笔迹复核。
    static let inkPressureWidthGainRatio: Double = 0.098
