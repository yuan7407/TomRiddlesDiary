...
| 笔画引擎 | ✅ 手绘化 + 压感 + 严格串行逐笔重播，211 个 XCTest 全通过 |
...
预期 `211 tests, 0 failures` 与 `** BUILD SUCCEEDED **`。构建有一条非阻塞提示（未依赖 AppIntents，跳过其 metadata），属预期。门禁会报若干占位品牌名的 ⚠，在白名单内，属预期。
...
| 编号 | 内容 |
|---|---|
| C1 | 先定评测集再调 prompt：8–10 段**文字**样张（你会写进日记的那种话，直接打字即可）+ 每段期望读出什么、绝对不该说什么。**刻意不是手写图片**：Oracle 只收得到文字，识别准不准是另一条链路（属 E4），两件事混在一起会让模型对比结论被识别错误污染 |
