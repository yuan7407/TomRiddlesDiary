...
| E6d（接真 provider） | ⬜ **等你两样：①选哪家 ②key + 实际 Base URL 与模型名**。C1 参考答案定稿后先写 system prompt，再按 C1 的 11 段逐家实测。**永久 key 绝不进客户端**，本地 DEBUG 若用 dev key 必须走 gitignored 配置且该 build 永不分发 |
