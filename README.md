...
| **E9（回应落点）** | 用户已定方向：回应排在刚写完那句话旁边，找个空位。拆成五步，下面四步已完成 |
...
| E9e（空位搜索与落点决策） | ⬜ 还差这一步。输入全齐了（这一轮写在哪、哪里是空的、字多大、按某宽度占多大），剩下的是决策：候选位置按「右边 → 下方 → 页面其他地方」排序，同样好的位置之间随机挑；宽度从舒服的值起，放不下就收窄重试。它一完成，E6（Oracle）与 E3f（写满翻页）就都不再被挡着 |
| E5 | 端侧提取字迹指标 |
