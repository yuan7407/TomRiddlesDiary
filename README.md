...
| E6a（协议 + Mock + 接线） | ✅ 2026-09-01。`OracleProvider` 协议 + `OracleRequest`/`OracleReply`/`OracleFailure`、DEBUG 专用的 `MockOracleProvider`、`ReplyComposer`（按「定落点 → 排版 → 手绘化」的固定顺序装配），并接进 `DiaryPageModel`。**那条断了很久的链路现在通了**：写字 → 成页 → 识别 → 问魂 → 定落点 → 排笔画 → 逐笔写。Mock 只在 DEBUG 存在，Release 没配 provider 就走 `.soulSilent(.notConfigured)`，纸上如实说魂接不上。顺带把识别器抽成 `HandwritingReading` 协议——真识别器读不出测试里造的合成笔画，不抽出来「问魂」这一环在测试里永远走不到 |
| E6b（危机场景怎么写出来） | ⬜ 待你拍板。C1 第 11 段的回应约 70 字，按 1.5 字/秒要写将近一分钟，让处于那种状态的人等一分钟才看到求助电话是坏设计。三个方向见 `C1-response-quality-set.md` 第 11 段，我倾向 E6b-2（先写号码那两行） |
| E6c（回应变长之后要不要调快书写速度） | ⬜ 待你拍板。加了邀请之后回应是 25–40 字 = 写 16–27 秒。三个方向见 C1 文档「回应有多长」一节，我倾向先按现状跑一次再定 |
| E6d（接真 provider） | ⬜ 等你提供 API key。C1 参考答案定稿后先写 system prompt，再按 C1 的 11 段逐家实测。**永久 key 绝不进客户端**，本地 DEBUG 若用 dev key 必须走 gitignored 配置且该 build 永不分发 |
