...
预期 `144 tests, 0 failures` 与 `** BUILD SUCCEEDED **`。构建有一条非阻塞提示（未依赖 AppIntents，跳过其 metadata），属预期。门禁会报若干占位品牌名的 ⚠，在白名单内，属预期。
...
| E1c（标点笔画） | ⬜ 数据集不含标点，需自己定义（约十几个符号） |
| E1d（拉丁与数字笔画） | ⬜ 数据集不含，需自己录（约 62 个符号，有限封闭集合） |
