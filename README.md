...
| A11（魂写字比真人快多少） | ✅ 2026-09-01 用户拍板 1.5 倍。**整段时间缩放**（`StrokeSequence.timeScaled`）而不是调「书写速度」那个参数：一段回应的时长 = 落墨 + 笔间抬笔移动 + 每次抬落笔的固定耗时，只调书写速度的话固定耗时不跟着变（实测 189 笔的回应里它占约 9 秒 / 37 秒），整体快不到 1.5 倍而且三者比例被改了——那是在改手感不是改速度。实测 33 字从 37 秒降到约 25 秒。这个值**是产品手感选择不是测量结果**，和 A10 要校准的那批参数不是一回事 |
...
| E3f（页面写满时翻页） | ✅ 2026-09-01。**判据是落点决策找不到空位**（`noRoomOnThisPage`），不是「occupancy 超过百分之几」——要紧的不是纸上有多少墨，而是「这段话还放得下吗」，而一段 30 字和一段 3 字的回应对空间的要求差很多。新建 `DiaryBook`（多页 + 当前页），写满自动翻到新页并在那儿写；旧页留在 `pages` 里不丢。**不循环翻页**：一段回应在全新空白纸上都放不下就如实报错，否则只会翻出一叠空白页。动画是横向滑动（旧页左移出、新页右移入，0.45 秒），画布与魂的字整块一起走 |
| E3f-2（真正的翻卷动画） | ⬜ 现在是横向滑动。3D 翻卷要自己做三维变换与阴影，是一整块活，而滑动已经把「换了一张纸」说清楚了。等这一版手感被实际用过之后再决定值不值得做 |
| E3g（魂的字落定进 PKDrawing） | ⬜ 决策 18 要求用户能用系统橡皮擦掉魂写的字，现在擦不掉。卡在一个真冲突上：轮次按**落笔时刻**切，而魂的笔画时刻也是「现在」，直接塞进 `PKDrawing` 会让魂读到自己写的话。要先解决那个冲突 |
