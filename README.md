...
| 笔画引擎 | ✅ 手绘化 + 压感 + 严格串行逐笔重播，226 个 XCTest 在 iOS 26.5 与 27.0 上都通过 |
...
预期 `226 tests, 0 failures`（在 iPadOS 26 的模拟器上会有 4 条识别用例如实跳过） 与 `** BUILD SUCCEEDED **`。构建有一条非阻塞提示（未依赖 AppIntents，跳过其 metadata），属预期。门禁会报若干占位品牌名的 ⚠，在白名单内，属预期。
